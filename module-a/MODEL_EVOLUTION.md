...
> **📌 当前线上版本：`monitor-v2.0`**  
> 最佳端点 RMSE：**26.17** cycles | 参数量：~863k | 架构：TCN-BiGRU + Deep SVDD 异常头  
> v2 升级详情见 [`README.md`](README.md) 第 7-8 节。本文档记录 v0→v1 进化过程，v1→v2 进化在 README 中。
...
| **v2 多任务升级（Bitahub 4090）** | **≈ 26.17** | **已晋级：异常+双归因** |
| 相对提升 | **约 6.5× 误差下降** | 主要来自数据管道与评估协议，不是堆更大网络 |

当前线上模型：`monitor-v2.0`（已由 v1 晋级）  
检查点：`model/saved/best_model.pt`（schema v2，epoch 25）  
结构：TCN-BiGRU + RUL Head + Anomaly Head，约 **86.3 万** 参数，输入 **34** 维 × 窗口 **30**。
