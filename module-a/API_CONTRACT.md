...
> **📌 v2 更新**：当前线上版本 `monitor-v2.0` 已补全以下字段：
> `anomaly_score`（0–1 分位数校准）、`anomaly_type`（`normal` / `condition_representation_deviation`）、
> `feature_attribution`（RUL Integrated Gradients）、`anomaly_attribution`（异常 IG）、`attribution_metadata`。
> 详细差异见 `README.md` 第 9 节。
>
> **以下为 v1 契约原文，保留作为历史记录。** v1 的 `anomaly_*=null` 已不适用。
